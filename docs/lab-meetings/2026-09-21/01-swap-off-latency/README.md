...
## 결과 3 — 같은 용량인데 애플리케이션 데이터는 512 vs 712MiB (원인 미규명)

> ⚠️ **정정 이력**: 이 절은 처음에 "swap으로 채운 용량은 **실효 용량이 28%
> 적다**"라고 썼으나 **틀린 설명이라 정정했다.** 확인해보니 두 조건 모두
> 준 용량을 99% 다 썼다 — 용량이 낭비된 것이 아니다.

두 조건 모두 명목 용량을 거의 완전히 소진하고 죽었다:

| | `memory.current` | `swap.current` | 합계 | 명목 용량 | 사용률 |
|---|---:|---:|---:|---:|---:|
| A | 255.9MiB | 503.3MiB | 759.2MiB | 768MiB | **98.9%** |
| D | 767.8MiB | 0 | 767.8MiB | 768MiB | **99.97%** |

차이는 **그 용량을 무엇으로 채웠는가**였다:

| | 청크(애플리케이션 데이터) | 청크 외 | JVM 커밋 힙 | 커밋 힙 중 데이터 비율 |
|---|---:|---:|---:|---:|
| A | 512MiB | **247MiB** | 758MiB | **68%** |
| D | 712MiB | **56MiB** | 758MiB | **94%** |

**두 JVM 모두 힙을 758MiB로 동일하게 커밋했는데**, A는 그중 실제 데이터가
68%뿐이었고 나머지 247MiB는 비었거나 쓰레기인 채로 용량을 점유하다 죽었다.

즉 애플리케이션 관점에서 보면 **같은 768MiB를 줘도 swap으로 준 쪽은 데이터를
512MiB밖에 담지 못했다**(RAM은 712MiB). 이건 실제로 관측된 차이지만,
**원인은 검증하지 못했다.**

- 가설(미검증): swap 압박 하에서 GC가 회수·압축을 제대로 못 해 빈 힙이
  누적됐을 가능성
- 가설(미검증): 커밋된 힙 페이지가 GC 과정에서 터치되며 swap으로 밀려나
  실제 데이터용 공간을 잠식했을 가능성
- 이를 구분하려면 **GC 로그(`-Xlog:gc*`) 수집**이 필요하다 — 이번엔 미수집
...
3. **같은 용량(768MiB)인데 애플리케이션 데이터는 512 vs 712MiB로 차이가 났다.**
   단 두 조건 모두 용량은 99% 소진했으므로 "용량이 낭비됐다"는 뜻은 아니며,
   커밋된 힙(둘 다 758MiB) 중 실제 데이터 비율이 68% vs 94%로 달랐다.
   **원인은 미규명**(GC 로그 미수집).
...
- A의 커밋 힙 중 데이터 비율이 68%에 그친 원인 미규명 — `-Xlog:gc*`로 GC
  로그를 수집해야 구분 가능
