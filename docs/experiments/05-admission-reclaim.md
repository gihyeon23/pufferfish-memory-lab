...
- **reclaim 대상 선정**: `reclaim_host()`를 그대로 재사용하므로 그 선정
  정책을 그대로 물려받는다. 5차 구현 당시엔 "가장 최근에 puff된 컨테이너
  부터"(mtime 기준)였지만, **6차에서 EJF(가장 나중에 생성된 컨테이너부터)로
  교체됐다** — 아래 "발견된 버그" 절의 재현 로그는 이미 6차 적용 후 코드로
  테스트한 것이라 EJF 순서로 나온다. `admission.py` 자체는 이 교체 때
  수정할 필요가 없었다(정책 로직이 `reclaim_host()` 안에 캡슐화돼 있어서).
